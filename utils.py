...
def segment_by_color(
    ...
    spatial_weights: list[float] = [4.8, 1, 1],  # Controls importance of each features [x, y, hue_x, hue_y, value]
    visualise = False
    ...
    Args:
        ...
        spatial_weight: Relative importance of each features
    ...
    for i in range(len(features[0])):
        features_scaled[:, i] *= spatial_weights[i]  # re-apply spatial weight after scaling
    ...
    for label in unique_labels:
        ...
        # Create the cluster image: white background, cluster pixels shown
        cluster_img = np.full_like(img, 255, dtype=np.uint8)
